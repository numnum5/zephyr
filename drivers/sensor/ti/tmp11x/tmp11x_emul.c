...
#define TMP11X_CFGR_RO_MASK \
	(TMP11X_EEPROM_UL_UNLOCK | \
	 TMP11X_EEPROM_UL_BUSY | \
	 TMP11X_CFGR_DATA_READY | \
	 BIT(0))
...
{
	struct tmp11x_emul_data *data = (struct tmp11x_emul_data *)target->data;
...
static int tmp11x_emul_transfer_i2c(const struct emul *target, struct i2c_msg *msgs,
		int num_msgs,
		int addr)
{
	...
	case 1:
		...
		if (msg0->len != 3) {
			LOG_ERR("Unexpected msg0 length %d",
				msg0->len);
			return -EIO;
		}

		reg = msg0->buf[0];
		val = ((uint16_t)msg0->buf[1] << 8) |
		       msg0->buf[2];
	...
	case 2:
		...
		/* First message must be register select write */
		if (msg0->flags & I2C_MSG_READ) {
		...
		if (msg1->len != 2) {
			LOG_ERR("Unexpected msg1 length %d",
				msg1->len);
			return -EIO;
		}
				   ...
				   const q31_t *value, int8_t shift)
{
	...
	data->reg[TMP11X_REG_TEMP] = raw_temp;
	data->reg[TMP11X_REG_CFGR] |= TMP11X_CFGR_DATA_READY;
...
#define TMP11X_EMUL(n)                                                                          \
	...
	EMUL_DT_INST_DEFINE(n, tmp11x_emul_init, &tmp11x_emul_data_##n,                      \
			    &tmp11x_emul_cfg_##n, &tmp11x_emul_api_i2c,                      \
			    &tmp11x_emul_sensor_driver_api)
